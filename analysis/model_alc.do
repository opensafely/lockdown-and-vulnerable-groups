...
/*					output/alc_ratioLoess1.svg				*/
/*					output/alc_ratioLoess2.svg				*/
/*					output/alc_plot1.svg						*/
/*					output/alc_plot2.svg						*/
/*					output/alc.dta								*/
/*					output/alc_ld1.dta							*/
/*					output/alc_ld2.dta							*/
/*					output/CITS_alc.xlsx						*/

/* Purpose:			Run CITS models of GP contact rates through	*/
/*					Covid lockdowns for people with alcohol	 	*/
...
graph export "$dir/output/alc_ratioLoess1.svg", replace
...
predict alc_yhat
gen alc_pred_rate=alc_yhat/population
predict res, pearson
predict error, stdp
generate ll=alc_pred_rate - invnormal(0.975)*error
generate ul=alc_pred_rate + invnormal(0.975)*error
...
list alc_yhat alc_pred_rate res error population value ul ll if _z==1&_n<10

save "$dir/output/alc_ld1.dta", replace

* model diagnostics
graph twoway (scatter res alc_pred_rate), title("Pearson residuals vs. predicted rates") yline(0) name(graph1, replace)
...
graph twoway (scatter value alc_pred_rate) (line value value), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("Alcohol misuse diagnostics - 1st lockdown")

graph export "$dir/output/diagnostics/alc_diagnostics1.svg", replace
...
(line alc_pred_rate date2 if _z==0, lcolor(gray)) ///
(line alc_pred_rate date2 if _z==1, lcolor(black)), ///
...
graph export "$dir/output/alc_plot1.svg", replace
...
graph export "$dir/output/alc_ratioLoess2.svg", replace
...
*postestimation values for plotting
predict alc_yhat2
gen alc_pred_rate2=alc_yhat2/population
predict res2, pearson
predict error2, stdp
generate ll2=alc_pred_rate2 - invnormal(0.975)*error2
generate ul2=alc_pred_rate2 + invnormal(0.975)*error2
...
save "$dir/output/alc_ld2.dta", replace

* model diagnostics
graph twoway (scatter res2 alc_pred_rate2), title("Pearson residuals vs. predicted rates") yline(0) name(graph1, replace)
...
graph twoway (scatter value alc_pred_rate2) (line value value), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("Alcohol misuse diagnostics - 2nd & 3rd lockdowns")

graph export "$dir/output/diagnostics/alc_diagnostics2.svg", replace
...
graph export "$dir/output/alc_plot2.svg", replace
