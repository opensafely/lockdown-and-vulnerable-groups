...
graph twoway (line intdis_pred_rate time if _z==1, lcolor(black)) (line intdis_pred_rate time if _z==0, lcolor(gray)) (scatter value time if _z==1, mcolor(black) msymbol(o)) (scatter value time if _z==0, mcolor(gray) msymbol(o)), legend(order(1 "Intervention estimate" 2 "Control estimate" 3 "Intervention rates" 4 "Control rates")) xline(0, lcolor(black) lpattern(dash)) xline(8, lcolor(black) lpattern(dash)) xscale(range(-30 32)) xlabel(-30 -20 -10 0 10 20 30) yscale(range(0 0.25))

graph export "$dir/output/intdis_plot1.pdf", as(pdf) replace
...
gen intdis_pred_rate2=intdis_yhat2/population

graph twoway (line intdis_pred_rate2 time if _z==1, lcolor(black)) (line intdis_pred_rate2 time if _z==0, lcolor(gray)) (scatter value time if _z==1, mcolor(black) msymbol(o)) (scatter value time if _z==0, mcolor(gray) msymbol(o)), legend(order(1 "Intervention estimate" 2 "Control estimate" 3 "Intervention rates" 4 "Control rates")) xline(33, lcolor(black) lpattern(dash)) xline(54, lcolor(black) lpattern(dash)) xscale(range(8 79)) xlabel(10 20 30 40 50 60 70 80) yscale(range(0 0.25))

graph export "$dir/output/intdis_plot2.pdf", as(pdf) replace