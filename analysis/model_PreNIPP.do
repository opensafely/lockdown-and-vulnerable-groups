...
graph export "$dir/output/dva_plot1.pdf", as(pdf)


** CITS model for second and thrid lockdowns

* run itsa initially to get dummy variables
xi: itsa consultations i.month, treat(1) trperiod(62 83) replace, if date2>=d(11may2020)&date2<=d(20sep2021)
...
* Change point 1: start of lockdown
*   step change = _z_x30 
*   slope change
...
predict dva_yhat
gen dva_pred_rate=dva_yhat/population
graph twoway (line dva_pred_rate time if group==1, lcolor(black)) (line dva_pred_rate time if dva==0, lcolor(gray)) (scatter value time if dva==1&date2>=d(11may2020)&date2<=d(20sep2021), mcolor(black) msymbol(o)) (scatter value time if dva==0&date2>=d(11may2020)&date2<=d(20sep2021), mcolor(gray) msymbol(o)), legend(order(1 "Intervention estimate" 2 "Control estimate" 3 "Intervention rates" 4 "Control rates")) xline(0, lcolor(black) lpattern(dash)) xline(8, lcolor(black) lpattern(dash)) xscale(range(-30 32)) xlabel(-30 -20 -10 0 10 20 30)
...
predict dva_yhat
gen dva_pred_rate=dva_yhat/population
graph twoway (line dva_pred_rate time if group==1, lcolor(black)) (line dva_pred_rate time if dva==0, lcolor(gray)) (scatter value time if dva==1, mcolor(black) msymbol(o)) (scatter value time if dva==0, mcolor(gray) msymbol(o)), legend(order(1 "Intervention estimate" 2 "Control estimate" 3 "Intervention rates" 4 "Control rates")) xline(0, lcolor(black) lpattern(dash))
graph export "$dir/output/dva_plot.pdf", as(pdf)
