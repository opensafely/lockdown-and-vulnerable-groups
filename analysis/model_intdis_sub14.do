...
/* Data used:		output/intdis_sub14.dta					*/
					
/* Outputs:			analysis/diagnostics/intdis_diagnostics_jr1.svg	*/
/*					analysis/diagnostics/intdis_diagnostics_jr2.svg	*/
/*					output/intdis_ratioLoess_jr1.svg				*/
/*					output/intdis_ratioLoess_jr2.svg				*/
/*					output/intdis_sub14_plot1.svg					*/
/*					output/intdis_sub14_plot2.svg					*/
/*					output/intdis_sub14_2.dta						*/
/*					output/intdis_sub14_2_ld1.dta					*/
/*					output/intdis_sub14_2_ld2.dta					*/
/*					output/CITS_intdis_sub14.xlsx					*/
...
*Get data
use "$dir/output/intdis_sub14.dta", clear
...
save "$dir/output/intdis_sub14_2.dta", replace

*summary stats
tabstat consultations_sub14 population_sub14 if _z==0, statistics(mean) by(period) format(%20.2f)
tabstat consultations_sub14 population_sub14 if _z==1, statistics(mean) by(period) format(%20.2f)
...
generate rr=value_sub14/control_rate
...
graph export "$dir/output/intdis_ratioLoess_jr1.svg", replace


** Main model: NegBin regression using variables defined above: z=group x=period(pre/post) t=time
xi: glm consultations_sub14 i.month xmas ny easter pubhol _t _z _z_t _x30 _x_t30 _z_x30 _z_x_t30 _x37 _x_t37 _z_x37 _z_x_t37, family(nb ml) link(log) exposure(population_sub14) vce(robust)

*export model outputs
putexcel set "$dir/output/CITS_intdis_sub14.xlsx", sheet("Intdis_sub14_1") replace
putexcel A1=matrix(r(table)), names 

*postestimation values for plotting
predict intdis_yhat
gen intdis_pred_rate=intdis_yhat/population_sub14
predict res, pearson
predict error, stdp
generate ll=(intdis_yhat - invnormal(0.975)*error)/population_sub14
generate ul=(intdis_yhat + invnormal(0.975)*error)/population_sub14

list intdis_yhat intdis_pred_rate res error population_sub14 value_sub14 ul ll if _z==1&_n<10

save "$dir/output/intdis_sub14_2_ld1.dta", replace
...
graph twoway (scatter value_sub14 intdis_pred_rate) (line value_sub14 value_sub14), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("Intellectual disability (<14) diagnostics - 1st lockdown")

graph export "$dir/output/diagnostics/intdis_diagnostics_jr1.svg", replace

* plot observed and predicted values
graph twoway (rarea ll ul date2 if _z==1, sort lcolor(gray) fcolor(gs11) lwidth(0)) ///
(scatter value_sub14 date2 if _z==0, mcolor(gray) msymbol(o)) ///
(scatter value_sub14 date2 if _z==1, mcolor(black) msymbol(o)) ///
...
graph export "$dir/output/intdis_sub14_plot1.svg", replace


/*** CITS model for second and third lockdowns ***/

use "$dir/output/intdis_sub14_2.dta", clear

drop if date2<d(11may2020)|date2>d(20sep2021)


**Simple model of rate ratio
generate rr2=value_sub14/control_rate
...
graph export "$dir/output/intdis_ratioLoess_jr2.svg", replace


** Main model: NegBin regression using variables defined above: z=group x=period(pre/post) t=time
xi: glm consultations_sub14 i.month xmas ny easter pubhol _t _z _z_t _x62 _x_t62 _z_x62 _z_x_t62 _x83 _x_t83 _z_x83 _z_x_t83, family(nb ml) link(log) exposure(population_sub14) vce(robust)

*export model outputs
putexcel set "$dir/output/CITS_intdis_sub14.xlsx", sheet("intdis_sub14_2") modify
putexcel A1=matrix(r(table)), names 

*postestimation values for plotting
predict intdis_yhat2
gen intdis_pred_rate2=intdis_yhat2/population_sub14
predict res2, pearson
predict error2, stdp
generate ll2=(intdis_yhat2 - invnormal(0.975)*error2)/population_sub14
generate ul2=(intdis_yhat2 + invnormal(0.975)*error2)/population_sub14

list value_sub14 ul2 ll2 if _z==1&_n<10

save "$dir/output/intdis_sub14_2_ld2.dta", replace
...
graph twoway (scatter value_sub14 intdis_pred_rate2) (line value_sub14 value_sub14), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("Intellectual disability (<14) diagnostics - 2nd & 3rd lockdowns")

graph export "$dir/output/diagnostics/intdis_diagnostics_jr2.svg", replace
...
graph export "$dir/output/intdis_sub14_plot2.svg", replace
