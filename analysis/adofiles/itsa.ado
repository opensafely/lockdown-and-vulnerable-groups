...
version 13.0
	...
	quietly {
		...
		*/* New error 5 */
		*if `multi_pnl'== 0 & "`single'"=="" {
		*	di as error " option single required for one panel analysis"
		*	exit 498
		*}

	 /********************* SET ANALYSIS TYPES ******************
	  *		  Type 1: Single panel in data set					 *
	  *		  Type 2: Multiple panels:	analyze one				 *
	  *		  Type 3: Multiple panels: analyze one vs controls	 *
	  ************************************************************/
		if `multi_pnl'==0 {
			local atype = 1
		}
		...
		/* verify the use of controls and generate inlist for use in newey */
		if "`contid'" != "" {
	...
	/*************************************************
	  TYPE 1 ANALYSIS: SINGLE GROUP IN DATASET
	**************************************************/
	if `atype'==1 {
		sort `touse' `tvar'
		...
		/* not _n as there may be gaps    */
		 ...
		 quietly gen `prefix'_t = `tvar' - `tvar'[1] + 1 if `touse'
		 local rhs `prefix'_t // collating RHS variables

		quietly foreach t in `trperiod' {
		...
		tsset
		if "`prais'" != "" {
			prais `dvar' `xvar' `rhs' if `touse' , `options'
		}
		else newey `dvar' `xvar' `rhs' if `touse' [`weight' `exp'], lag(`lag') `options'

		/*************************************************************
		*  LINCOM FOR SINGLE GROUP SINGLE PANEL                     *
		*************************************************************/
		if "`posttrend'" != ""{
		...
		if "`xvar'"!=""{   /* covariates present */
			local plotvars `ypred_t'
		 }
		else {
		...
		noi scatter  `dvar' `plotvars'   `tvar' if `touse',
			`cpart' `mspart' `lc'
	...
	/* TYPE 2  MULTIPLE PANELS, ANALYZE ONE*/
		...
		bysort `touse' `pvar' (`tvar'): gen `prefix'_t = `tvar' - `tvar'[1] + 1 if `touse'
		local rhs `prefix'_t // collating RHS variables
		...
		tsset
		if "`prais'" != "" {
			prais `dvar' `xvar' `rhs' if `touse' & `pvar'==`treatid' , `options'
		}
		else newey `dvar' `xvar' `rhs' if `touse' & `pvar'==`treatid' [`weight' `exp'], lag(`lag') `options'
 ...
 *************************************************************/
	...
	if "`figure'" != ""{   /* Start Figure Loop */
		...
		scatter  `dvar' `plotvars'   `tvar' if `pvar'==`treatid' & `touse',
			`cpart' `mspart' `lc'
			xline(`trperiod', lpattern(shortdash) lcolor(black))
			mcolor(black)
			legend(rows(1) order(1 2)
...
**************************************************/
		...
		tsset
		if "`prais'" != "" {
			prais `dvar' `xvar' `rhs'  if `touse' `if2' , `options'
		}
		else newey `dvar' `xvar' `rhs' `x_treat' if `touse' `if2' [`weight' `exp'], lag(`lag') force `options'
		quietly predict `prefix'_m_`dvar'_pred
		...
		********************************************/
		...
		if "`figure'" != ""{   /* Start Figure Loop */
			...
			if "`prais'" !="" {
			 ...
			 preserve
			/* Collapse actual&predicted to treat/control means */
			collapse (mean) `dvar' `prefix'_m_`dvar'_pred ///
				if `touse' `if2', by(`tvar' `prefix'_z)
