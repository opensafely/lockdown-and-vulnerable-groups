...
*summary stats
tabstat consultations_f population_f if _z==0, statistics(mean) by(period) format(%20.2f)
tabstat consultations_f population_f if _z==1, statistics(mean) by(period) format(%20.2f)
...
** Main model: NegBin regression using variables defined above: z=group x=period(pre/post) t=time
glm consultations_f i.month xmas ny easter pubhol _t _z _z_t _x62 _x_t62 _z_x62 _z_x_t62 _x83 _x_t83 _z_x83 _z_x_t83, family(nb ml) link(log) exposure(population_f) vce(robust)
