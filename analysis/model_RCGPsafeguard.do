...
/*					output/safeguard_ratioLoess1.svg				*/
/*					output/safeguard_ratioLoess2.svg				*/
/*					output/safeguard_plot1.svg						*/
/*					output/safeguard_plot2.svg						*/
/*					output/safeguard.dta							*/
/*					output/safeguard_ld1.dta						*/
/*					output/safeguard_ld2.dta						*/
/*					output/CITS_safeguard.xlsx						*/

/* Purpose:			Run CITS models of GP contact rates through	*/
/*					Covid lockdowns for children with 		 	*/
...
save "$dir/output/safeguard.dta", replace
...
graph export "$dir/output/safeguard_ratioLoess1.svg", replace
...
putexcel set "$dir/output/CITS_safeguard.xlsx", sheet("safeguard1") replace
putexcel A1=matrix(r(table)), names 

*postestimation values for plotting
predict safeguard_yhat
...
save "$dir/output/safeguard_ld1.dta", replace

* model diagnostics
graph twoway (scatter res safeguard_pred_rate), title("Pearson residuals vs. predicted rates") yline(0) name(graph1, replace)
...
graph twoway (scatter value safeguard_pred_rate) (line value value), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("RCGP safeguarding diagnostics - 1st lockdown")

graph export "$dir/output/diagnostics/safeguard_diagnostics1.svg", replace
...
(line safeguard_pred_rate date2 if _z==0, lcolor(gray)) ///
(line safeguard_pred_rate date2 if _z==1, lcolor(black)), ///
...
graph export "$dir/output/safeguard_plot1.svg", replace


/*** CITS model for second and third lockdowns ***/

use "$dir/output/safeguard.dta", clear
...
graph export "$dir/output/safeguard_ratioLoess2.svg", replace
...
*export model outputs
putexcel set "$dir/output/CITS_safeguard.xlsx", sheet("safeguard2") modify
putexcel A1=matrix(r(table)), names 

*postestimation values for plotting
predict safeguard_yhat2
...
* model diagnostics
graph twoway (scatter res2 safeguard_pred_rate2), title("Pearson residuals vs. predicted rates") yline(0) name(graph1, replace)
...
graph twoway (scatter value safeguard_pred_rate2) (line value value), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("RCGP safeguarding diagnostics - 2nd & 3rd lockdowns")

graph export "$dir/output/diagnostics/safeguard_diagnostics2.svg", replace
...
graph export "$dir/output/safeguard_plot2.svg", replace
