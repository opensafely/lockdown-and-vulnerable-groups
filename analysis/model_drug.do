...
/* Data used:		output/measure_drugmisuse_rate.csv			*/
					
/* Outputs:			analysis/diagnostics/drug_diagnostics1.svg	*/
/*					analysis/diagnostics/drug_diagnostics2.svg	*/
/*					output/drug_ratioLoess1.svg				*/
/*					output/drug_ratioLoess2.svg				*/
/*					output/drug_plot1.svg						*/
/*					output/drug_plot2.svg						*/
/*					output/drug.dta								*/
/*					output/drug_ld1.dta							*/
/*					output/drug_ld2.dta							*/
/*					output/CITS_drug.xlsx						*/

/* Purpose:			Run CITS models of GP contact rates through	*/
/*					Covid lockdowns for people with drug	 	*/
/*					dependence, misuse or harmful use			*/
/****************************************************************/
...
sort drugmisuse time
by drugmisuse: gen trperiod=_n
...
save "$dir/output/drug.dta", replace
...
graph export "$dir/output/drug_ratioLoess1.svg", replace
...
putexcel set "$dir/output/CITS_drug.xlsx", sheet("drug1") replace
putexcel A1=matrix(r(table)), names 

*postestimation values for plotting
predict drug_yhat
gen drug_pred_rate=drug_yhat/population
predict res, pearson
predict error, stdp
generate ll=drug_pred_rate - invnormal(0.975)*error
generate ul=drug_pred_rate + invnormal(0.975)*error
...
save "$dir/output/drug_ld1.dta", replace

* model diagnostics
graph twoway (scatter res drug_pred_rate), title("Pearson residuals vs. predicted rates") yline(0) name(graph1, replace)
...
graph twoway (scatter value drug_pred_rate) (line value value), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("Drug misuse diagnostics - 1st lockdown")

graph export "$dir/output/diagnostics/drug_diagnostics1.svg", replace
...
(line drug_pred_rate date2 if _z==0, lcolor(gray)) ///
(line drug_pred_rate date2 if _z==1, lcolor(black)), ///
...
graph export "$dir/output/drug_plot1.svg", replace


/*** CITS model for second and third lockdowns ***/

use "$dir/output/drug.dta", clear
...
graph export "$dir/output/drug_ratioLoess2.svg", replace
...
*export model outputs
putexcel set "$dir/output/CITS_drug.xlsx", sheet("drug2") modify
putexcel A1=matrix(r(table)), names 

*postestimation values for plotting
predict drug_yhat2
gen drug_pred_rate2=drug_yhat2/population
predict res2, pearson
predict error2, stdp
generate ll2=drug_pred_rate2 - invnormal(0.975)*error2
generate ul2=drug_pred_rate2 + invnormal(0.975)*error2
...
save "$dir/output/drug_ld2.dta", replace

* model diagnostics
graph twoway (scatter res2 drug_pred_rate2), title("Pearson residuals vs. predicted rates") yline(0) name(graph1, replace)
...
graph twoway (scatter value drug_pred_rate2) (line value value), title("Observed vs. predicted rates") name(graph4, replace)
graph combine graph1 graph2 graph3 graph4, title("Drug misuse diagnostics - 2nd & 3rd lockdowns")

graph export "$dir/output/diagnostics/drug_diagnostics2.svg", replace
...
graph export "$dir/output/drug_plot2.svg", replace
